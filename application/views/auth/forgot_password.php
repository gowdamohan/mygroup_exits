            ...
            <form enctype="multipart/form-data" id="Verify-Mobile-Number" class="form-horizontal" data-parsley-validate method="post">
              ...
              <input type="hidden" name="mobile_number" id="mobileNumber">
                <span class="login100-form-logo">
                  <img width="100px" style="margin-bottom:0.8rem" class="img-responsive" src="<?php echo base_url().'assets/front/logo.png' ?>">
                </span>
                <div style="padding:1rem 1.5rem;border-radius:16px;text-align: center;">
                  <h4>Lets find your account</h4>
                  <h5>Enter your registered Mobile number</h5>
                </div>

                <div class="input-group">
                  <span class="input-group-addon"><i style="font-size: 22px;" class="fa fa-mobile"></i></span>
                    <input type="number" required="" autofocus="off"  placeholder="Enter mobile number / email" id="email_mobileNumber" data-type="mobileNumber" name="email_mobileNumber" class="form-control input-md">
                </div>
...
<script type="text/javascript">
  var timerOn = true;
    ...
    function send_otp(){
      var mobileNumber = $("#email_mobileNumber").val();
      if (mobileNumber == '') {
        $('#error_reset').show();
        $('#sms_error').html("Mobile number can't be empty!");
      }
 ...
 function resend_timer(remaining) {
    ...
    $.ajax({
      ...
      success: function(data) {
        ...
        if (retData != 0) {
            ...
            $('#error_reset').show();
            $('#sms_error').html('No account associated with '+mobileNumber);
        }
  ...
  $(".verify").click(function (e){
    ...
    var mobileNumber = $('#email_mobileNumber').val();
    $.ajax({
      url:'<?php echo site_url('auth/verify_otp_mobile_number/'); ?>',
      type:'post',
      ...
      success: function(data) {
        ...
        if (res != 0) {
           ...
           $('#mobileNumber').val(mobileNumber);
            url = '<?php echo site_url('auth/forgot_reset_password_new') ?>';
