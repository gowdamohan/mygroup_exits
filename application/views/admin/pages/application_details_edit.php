<div class="panel panel-default">
    ...
    <form enctype="multipart/form-data" method="post" id="students" action="" class="form-horizontal" data-parsley-validate >
        <div class="panel-body table-responsive">
           <div class="col-md-6 col-md-offset-2">
                ...
                <div class="form-group">
                  ...
                  <div class="col-md-8">
                    <select class="form-control" required name="group_name" onchange="get_group_wise_app()" id="groupName" >
                      ...
                      <option <?php if ($edit_apps_about->group_name =='My Apps') echo 'selected' ?> value="My Apps">My Apps</option>
                      <option <?php if ($edit_apps_about->group_name =='My Company') echo 'selected' ?> value="My Company">My Company</option>
                      <option <?php if ($edit_apps_about->group_name =='My Onine Apps') echo 'selected' ?> value="My Onine Apps">My Onine Apps</option>
                      <option <?php if ($edit_apps_about->group_name =='My Offline Apps') echo 'selected' ?> value="My Offline Apps">My Offline Apps</option>
                    </select>
                  </div>
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                    ...
                    <div class="col-md-8">
                        <input type="text" name="about_title" value="<?php echo $edit_apps_about->title ?>" class="form-control" id="about_title" >
                    </div>
                </div>
                ...
                <div class="form-group">
                    ...
                    <?php if (!empty($edit_apps_about->image)) { ?>
                        <div class="imagehide">
                            <label class="control-label col-sm-4" for="no_of_days">Preeview Image</label>
                            <div class="col-sm-8">
                               <img src="<?php echo $this->filemanager->getFilePath($edit_apps_about->image) ?>">
                              <input type="hidden" id="exit-Preview" value="<?php echo $edit_apps_about->image ?>" >
                            </div>
                        </div>
                    <?php } ?>
               ...
               <div class="form-group" id="textSummernote">
                  ...
                  <div class="col-sm-8">
                    <textarea class="summernote" required="" id="summernote"><?php echo $edit_apps_about->content ?></textarea>
                  </div>
                </div>
        ...
        <div class="panel-footer">
            <center>
               <input type="button" value="Upload" onclick="uploadAbout_apps_edit('<?php echo $edit_apps_about->id ?>')" id="up-btn" class="btn btn-primary">
...
<script type="text/javascript" src="<?php echo base_url();?>assets/admin/js/plugins/summernote/summernote.js"></script>
 ...
 <script type="text/javascript">
    $(document).ready(function(){
        var groupName = '<?php echo $edit_apps_about->group_name ?>';
        get_group_wise_app_edit(groupName);
    });

    function get_group_wise_app_edit(groupName) {
         var app_name = '<?php echo $edit_apps_about->app_name ?>';
        $.ajax({
          ...
          success: function(data) {
            ...
            for (var i = 0; i < res.length; i++) {
                var selected = '';
                if (app_name == res[i].name) {
                    selected = 'selected';
                }
              option += '<option '+selected+' value="'+res[i].name+'">'+res[i].name+'</option>';
            }
...
function uploadAbout_apps_edit(aboutId) {
    ...
    $.ajax({
      url: '<?php echo site_url('admin_controller/upload_about_apps_by_id') ?>',
      type: 'post',
      data: form_data,
      cache: false,
      contentType: false,
