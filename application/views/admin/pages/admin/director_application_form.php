<ul class="breadcrumb">
  ...
  <li class="active">Create Member Application form fields</li>
</ul>
<div class="panel panel-default">
  <div class="panel-heading">
    <h3 class="panel-titile">Create Member Application form fields</h3>
  </div>
  <div class="panel-body">
    <form enctype="multipart/form-data" method="post" action="<?php echo site_url('admin_controller/director_applications_form') ?>" class="form-horizontal" data-parsley-validate >
  ...
  <form enctype="multipart/form-data" method="post" action="<?php echo site_url('admin_controller/insert_director_application_config_fields_admin/'.$unionCat);?>" data-parsley-validate="" class="form-horizontal">
        <div class="panel-body">
                ...
                foreach ($enabled_fields as $key => $val) { ?>
                    <tr>
                      <td style="vertical-align: middle; text-align: center;">
                        <input type="checkbox" class="enabledFields" <?php if (in_array($val, $selected_enabled_fields)) echo 'checked' ?> name="show_enabled_fields_director[]" value="<?php echo $val ?>" onclick="onclick_enabled_fields('<?php echo $val ?>')" style="width: 18px;height: 18px;" id="enabled<?php echo $val ?>" >
                      </td>
                      <td style="vertical-align: middle; text-align: center;">
                        <input type="checkbox" <?php if (in_array($val, $selected_required_fields)) echo 'checked' ?> name="required_fields_director[]" style="width: 18px;height: 18px;" value="<?php echo $val ?>" readonly="true" onclick="onclick_required_fields('<?php echo $val ?>')" id="required<?php echo $val ?>" >
                      </td>
                      <td>
                        <div class="form-group">
