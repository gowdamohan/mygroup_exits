<ul class="breadcrumb">
  ...
  <li class="active">Create Header Leader Application form fields</li>
</ul>
<div class="panel panel-default">
  <div class="panel-heading">
    <h3 class="panel-titile">Create Header Leader Application form fields</h3>
  </div>
  <div class="panel-body">
    <form enctype="multipart/form-data" method="post" action="<?php echo site_url('admin_controller/header_leader_applcation_form') ?>" class="form-horizontal" data-parsley-validate >
  ...
  <form enctype="multipart/form-data" method="post" action="<?php echo site_url('admin_controller/insert_header_leader_application_config_fields_admin/'.$unionCat);?>" data-parsley-validate="" class="form-horizontal">
        <div class="panel-body">
                  ...
                  <?php if ($val != 'present_area' && $val != 'present_district' && $val != 'present_state' && $val != 'present_country' && $val != 'present_pincode' && $val != 'permanent_area' && $val != 'permanent_district' && $val != 'permanent_state' && $val != 'permanent_country' && $val != 'permanent_pincode') { ?>
                    <tr>
                      <td style="vertical-align: middle; text-align: center;">
                        <input type="checkbox" class="enabledFields" <?php if (in_array($val, $selected_enabled_fields)) echo 'checked' ?> name="show_enabled_fields_header_leader[]" value="<?php echo $val ?>" onclick="onclick_enabled_fields('<?php echo $val ?>')" style="width: 18px;height: 18px;" id="enabled<?php echo $val ?>" >
                      </td>
                      <td style="vertical-align: middle; text-align: center;">
                        <input type="checkbox" <?php if (in_array($val, $selected_required_fields)) echo 'checked' ?> name="required_fields_header_leader[]" style="width: 18px;height: 18px;" value="<?php echo $val ?>" readonly="true" onclick="onclick_required_fields('<?php echo $val ?>')" id="required<?php echo $val ?>" >
                      </td>
                      <td>
                        <div class="form-group">
