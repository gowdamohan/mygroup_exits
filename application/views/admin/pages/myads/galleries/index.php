	...
	<div class="panel panel-default new-panel-style_3">
		<div class="panel-heading new-panel-heading">
			...
			<ul class="panel-controls" style="float:right">
				<li>
				<a  href="<?php echo site_url('myads/create_gallery');?>" data-placement="top" data-toggle="tooltip" data-original-title="Create a new Gallery" class="control-primary"><span class="fa fa-plus"></span></a>&nbsp; &nbsp;
				</li>
			</ul>
		</div>
		<div class="panel-body table-responsive" style="padding:8px 8px;">
			<table class='table table-bordered datatable' id='list_tab'>
				...
				<tbody>  
					...
					<td><?= $i++ ?></td>
						<td>
							<a href='<?php echo site_url('myads/view_gallery/'.$row['gallery_id']); ?>' >
								<?php echo $row['gallery_name']; echo ' -- ('.$row['image_count'].')'; ?>
							</a>
						</td>
						<td><center>
							<a href="<?php echo site_url('myads/view_gallery/'.$row['gallery_id']); ?>" class="btn  btn-success btn_align"
    ...
    callback: function (result) {
      if(result) {        
        $.ajax({
          ...
          url: '<?php echo site_url('myads/delete_gallery_by_id'); ?>',
          type: 'post',
          data: {'gId' : gId},
