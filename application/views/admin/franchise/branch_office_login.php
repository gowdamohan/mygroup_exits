<ul class="breadcrumb">
   ...
   <li>Branch Office Login</li>
</ul>
<div class="panel panel-default">
   <div class="panel-heading">
      <h3 class="panel-title">Branch Office Login</h3>
   </div>
   <div class="panel-body">
       <form enctype="multipart/form-data" action="<?php echo site_url('franchise/create_branch_office_login/') ?>" class="form-horizontal"  data-parsley-validate method="post" >
         <div class="col-md-3">
            <div class="form-group">
               <label class="control-label">Select State</label>
               <select onchange="form.submit()" class="form-control" name="state_id" id="state_id">
                  <option value="">Select State</option>
                  <?php foreach ($statelist as $key => $val) { ?>
                     <option <?php if($selected_state_id == $val->id) echo 'selected' ?> value="<?php echo $val->id ?>"><?php echo $val->state ?></option>
                  <?php } ?>
               </select>
            </div>
         </div>
      </form>
      <?php if (!empty($franchise_holder)) { ?>
            ...
            <tbody>
               <?php $i=1; foreach ($franchise_holder as $key => $val) { ?>
                  ...
                  if (!empty($val->user_id)) {
                     $profile_url = site_url('franchise/create_franchise_staff/'.$val->user_id.'/branch');
                  } ?>
                  <?php 
                     $disabled = 'readonly';
                     if (!empty($val->user_id)) {
                 ...
                 <tr>
                     <th><?php echo $i ?></th>
                     <th><a href="<?php echo $profile_url ?>"><?php echo $val->district ?></a></th>
                     <th><input type="text" class="form-control" <?php echo $disabled1 ?>  value="<?php echo $val->username ?>" name="username" id="username<?php echo $i ?>"></th>
                     ...
                     <input type="hidden" class="form-control" value="<?php echo $val->state_id ?>"  name="franchise_state" id="franchise_state<?php echo $i ?>">
                     <input type="hidden" class="form-control" value="<?php echo $val->id ?>" name="franchise_district" id="franchise_district<?php echo $i ?>">
                     ...
                     <th>
                        ...
                        <a <?php echo $disabled ?> class="btn btn-danger" href="<?php echo site_url('franchise/franchise_reset_password/'.$val->user_id.'/'.'create_branch_office_login') ?>">Reset password</a></th>
                  </tr>
               <?php $i++; } ?>
            </tbody>
         </table>
      <?php }else{ ?>
         <h3>Select State and get data</h3>
      <?php } ?>
...
<script type="text/javascript">
   function save_franchise_login_details(i) {
      ...
      var group_id = '8';
      var redirect_url = 'franchise/create_branch_office_login';
      $.ajax({
