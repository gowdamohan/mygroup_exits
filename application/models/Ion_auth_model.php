...
class Ion_auth_model extends CI_Model
{
	 ...
	 * @author Mathew
	 */
	public function identity_check_client($identity = '', $group_id)
	{
		$this->trigger_events('identity_check');
	...
	public function register($identity, $password, $email, $additional_data = array(), $groups = array())
	{
		...
		if ($this->identity_check($identity))
		{
			$this->session->set_flashdata('flashError', 'Mobile Number already exits.');
	 ...
	 * @author    Mathew
	 */
	public function client_login($identity, $password, $remember=FALSE, $groupName)
	{
		$this->trigger_events('pre_login');
		...
		$query = $this->db->select($this->identity_column . ', email, u.id, password, active, last_login, group_id')
				->from('users u')
	 ...
	 * @author    Mathew
	 */
	public function client_register_model($identity, $password, $email, $additional_data = array(), $groups = array(), $group_id)
	{
		$this->trigger_events('pre_register');

		$manual_activation = $this->config->item('manual_activation_client', 'ion_auth');

		if ($this->identity_check_client($identity, $group_id))
		{
			$this->session->set_flashdata('flashError', 'Mobile Number already exits.');
			// $this->set_error('account_creation_duplicate_identity');
			return FALSE;
	 ...
	 * @author    Mathew
	 */
	public function userlogin($identity, $password, $remember)
	{
		$this->trigger_events('pre_login');
