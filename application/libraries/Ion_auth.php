...
class Ion_auth
{
	 ...
	 * @author Mathew
	 */
	public function client_register($identity, $password, $email, $additional_data = array(), $group_ids = array(), $group_id)
	{
		$this->ion_auth_model->trigger_events('pre_account_creation');

		$email_activation = $this->config->item('email_activation', 'ion_auth');

		$id = $this->ion_auth_model->client_register_model($identity, $password, $email, $additional_data, $group_ids, $group_id);
	...
	public function user_login($identity, $password,$remember){
		return  $this->ion_auth->userlogin($identity, $password,$remember);
	}
